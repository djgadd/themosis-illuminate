 ...
 */
defined('COM_KELTIECOCHRANE_ILLUMINATE_TD') ? COM_KELTIECOCHRANE_ILLUMINATE_TD : define('COM_KELTIECOCHRANE_ILLUMINATE_TD', 'themosis-illuminate');
 ...
 */
$vars = [
  'slug' => 'themosis-illuminate',
  'name' => 'Themosis Illuminate',
  'namespace' => 'com.keltiecochrane.illuminate',
  'config' => 'com_keltiecochrane_illuminate',
];
...
add_action('admin_notices', function () use ($vars) {
  if (!class_exists('\Themosis\Foundation\Application')) {
    printf('<div class="notice notice-error"><p>%s</p></div>', __('This plugin requires the Themosis framework in order to work.', COM_KELTIECOCHRANE_ILLUMINATE_TD));
  }
  ...
  if (!current_theme_supports($vars['slug']) && current_user_can('switch_themes')) {
    printf('<div class="notice notice-warning"><p>%s<strong>%s</strong></p></div>', __('The current theme does not explicitly support the following plugin: ', COM_KELTIECOCHRANE_ILLUMINATE_TD), $vars['name']);
  }
});
...
$paths['plugin.'.$vars['namespace'].'.languages'] = __DIR__.DS.'languages'.DS;
...
container('action')->add('plugins_loaded', function () use ($vars) {
   ...
   */
  load_plugin_textdomain(COM_KELTIECOCHRANE_ILLUMINATE_TD, false, trailingslashit(dirname(plugin_basename(__FILE__))).'languages');
